...
- **`StringLiteral`**：字符串字面量。由正则 `'([^\'\n\r])*\'` 匹配，支持多字符和空字符串。在 Flex 动作中提取引号内的内容作为 lexeme（去掉了外围单引号）。
...
LexerResult Lexer::tokenizeDetailed(const std::string& source) {
    ...
    // 2. 创建 Flex 扫描缓冲区（从内存字符串扫描，使用 scan_bytes 避免 \0 截断）
    YY_BUFFER_STATE buf = pascclex_scan_bytes(source.c_str(), static_cast<int>(source.size()));
